...
This project demonstrates how to interface the **LIS2DUX12** accelerometer sensor with an **STM32F401RE** microcontroller board. The program configures the sensor FSM using a UCF file converted to Rust code, detects gesture events, and outputs notifications over UART.
...
- The sensor is configured by applying a sequence of register writes and delays generated by the **`ucf-tool`** from a UCF file (`lis2dux12_glance_detection.ucf`), which programs the sensor's FSM for gesture detection.
