...
impl<P, T> Lis2dux12<st_mems_bus::i2c::I2cBus<P>, T>
where
    P: I2c,
    T: DelayNs,
{
...
impl<P, T> Lis2dux12<st_mems_bus::spi::SpiBus<P>, T>
where
    P: SpiDevice,
    T: DelayNs,
{
...
impl<B, T> MemBankFunctions<MemBank> for Lis2dux12<B, T>
where
    B: BusOperation,
    T: DelayNs,
{
...
impl<B, T> EmbAdvFunctions for Lis2dux12<B, T>
where
    B: BusOperation,
    T: DelayNs,
{
...
impl<B: BusOperation, T: DelayNs> Lis2dux12<B, T> {
    #[inline]
    ...
    pub fn temp_disable_set(&mut self, val: u8) -> Result<(), Error<B::Error>> {
    ...
    pub fn temp_disable_get(&mut self) -> Result<u8, Error<B::Error>> {
        Ok(SelfTest::read(self)?.t_dis())
    }
    ...
    pub fn exit_deep_power_down(&mut self) -> Result<(), Error<B::Error>> {
        ...
        val.write(self)?;
        self.tim.delay_ms(25); // See AN5909 - paragraphs 3.1.1.1 and 3.1.1.2
        Ok(())
    }
    ...
    pub fn outt_data_get(&mut self) -> Result<OuttData, Error<B::Error>> {
        let raw = OutT::read(self)?.outt().cast_signed();
